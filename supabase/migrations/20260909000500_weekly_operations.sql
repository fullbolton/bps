-- NOT APPLIED TO PRODUCTION. Read-only weekly pilot, existing RLS applies.
BEGIN;
SET LOCAL lock_timeout='5s';
CREATE FUNCTION public.ops_week(p_company_id uuid,p_week_start date)
...
END $$;
REVOKE ALL ON FUNCTION public.ops_week(uuid,date) FROM PUBLIC,anon;
GRANT EXECUTE ON FUNCTION public.ops_week(uuid,date) TO authenticated;
COMMIT;
