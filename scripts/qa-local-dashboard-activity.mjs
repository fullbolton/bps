...
try{
 ...
 const migration=readFileSync(new URL('../supabase/migrations/20260909002100_dashboard_activity.sql',import.meta.url),'utf8');
 ...
 console.log('PASS 02100 applied only to guarded synthetic local Supabase');
 ...
 const email=`activity-${randomUUID()}@example.test`,password=randomUUID()+'aA1!';
 ...
 try{
  ...
  for(let i=0;i<20;i++){response=await c.rpc('dashboard_activity',args);if(!response.error)break;await new Promise(r=>setTimeout(r,150));}
  assert.ifError(response.error);assert.ok(Array.isArray(response.data));assert.ok(response.data.length<=20);
  const parser=await importActualTypeScript(new URL('../src/lib/dashboard-activity.ts',import.meta.url));parser.parseDashboardActivity(response.data);
  console.log('PASS real authenticated RPC returns a bounded parseable feed');
  assert.ok((await c.rpc('dashboard_activity',{...args,p_tenant_id:id(2)})).error);
  sql(`UPDATE profiles SET role='ik' WHERE id='${uid}'`);assert.ok((await c.rpc('dashboard_activity',args)).error);
  sql(`UPDATE profiles SET role='yonetici' WHERE id='${uid}'; DELETE FROM tenant_memberships WHERE user_id='${uid}'`);assert.ok((await c.rpc('dashboard_activity',args)).error);
  const anon=createClient(s.API_URL,s.ANON_KEY,options);assert.ok((await anon.rpc('dashboard_activity',args)).error);
...
}catch(error){console.error('Activity local apply failed: '+error.message);process.exitCode=1;}
finally{release?.();}
