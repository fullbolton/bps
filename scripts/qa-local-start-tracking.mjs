...
try{
 ...
 console.log('PASS 02700 applied to guarded local Supabase');
 ...
 try{
  ...
  assert.ifError((await c.rpc('ops_start_execute',{...cmd,p_command_id:randomUUID(),p_expected_revision:1,p_action:'call',p_payload:{offset:0,outcome:'claimed_arrival',occurredAt:new Date().toISOString()}})).error);
  const confirm={...cmd,p_command_id:randomUUID(),p_expected_revision:2,p_action:'confirm',p_payload:{source:'branch',witness:'Synthetic supervisor',occurredAt:new Date().toISOString()}};
