...
try{
 ...
 sql("ALTER TABLE profiles ADD COLUMN IF NOT EXISTS email text NOT NULL DEFAULT '',ADD COLUMN IF NOT EXISTS unit text");
 const registration=readFileSync(new URL('../supabase/migrations/20260909002400_invited_account_registration.sql',import.meta.url),'utf8');
 if(sql("SELECT to_regprocedure('public.prepare_invited_profile(uuid,text)') IS NULL")==='t')sql(registration);
 else{const bodies=[...registration.matchAll(/CREATE (?:OR REPLACE )?FUNCTION [\s\S]*?\$\$;/g)].map(m=>m[0].replace(/^CREATE (?:OR REPLACE )?FUNCTION/,'CREATE OR REPLACE FUNCTION'));assert.equal(bodies.length,3);sql('BEGIN;'+bodies.join('\n')+'COMMIT;');}
 assert.equal(sql("SELECT count(*) FROM pg_trigger WHERE tgname='bps_registration_test_profile'"),'0');
 sql('CREATE TRIGGER bps_registration_test_profile AFTER INSERT ON auth.users FOR EACH ROW EXECUTE FUNCTION public.handle_new_user()');
 sql("NOTIFY pgrst,'reload schema';");
 ...
 try{
  for(const [client,role] of [[manager,'yonetici'],[recipient,'goruntuleyici']]){
   const email=`invite-${randomUUID()}@example.test`,password=randomUUID()+'aA1!';const r=role==='yonetici'?await root.auth.admin.createUser({email,password,email_confirm:true,app_metadata:{active_tenant:id(1)}}):await client.auth.signUp({email,password,options:{data:{role:'yonetici',is_platform_admin:true,unit:'yonetim'}}});assert.ifError(r.error);const uid=r.data.user.id;users.push({id:uid,email});
   if(role==='goruntuleyici'){assert.equal(sql(`SELECT role FROM profiles WHERE id='${uid}'`),'goruntuleyici');assert.equal(sql(`SELECT is_platform_admin FROM profiles WHERE id='${uid}'`),'f');console.log('PASS real Auth signup ignores role/admin metadata');}sql(`INSERT INTO profiles(id,role,display_name) VALUES('${uid}','${role}','Davet kabul testi') ON CONFLICT(id) DO NOTHING`);if(role==='yonetici')sql(`UPDATE profiles SET role='yonetici' WHERE id='${uid}'`);if(role==='yonetici')sql(`INSERT INTO tenant_memberships VALUES('${uid}','${id(1)}')`);
  ...
  assert.equal((await recipient.rpc('invitation_registration_allowed',{p_id:invitationId,p_token:token,p_email:users[1].email})).data,true);assert.ifError((await recipient.rpc('prepare_invited_profile',{p_id:invitationId,p_token:token})).error);
  ...
  sql('DROP TRIGGER bps_registration_test_profile ON auth.users');
...
}catch(e){console.error('Local registration acceptance failed: '+e.message);process.exitCode=1;}
finally{release?.();}
